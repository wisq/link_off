...
Deploying LinkOff (via Distillery) offers a few extra benefits:
...
* `config/prod.exs`: Your production config.  If you want the same config in your local development and deployed versions, then just copy `dev.exs` to `prod.exs`.  Otherwise, copy `link_off.exs` and edit the config as needed.
...
Once deployed, you'll want to start it.  There's generally two ways to do this.  From within the `~link_off/app/link_off` directory (or whatever user/path you've deployed to):

* `bin/link_off start` will start LinkOff as a daemon (in the background).  You can automate this with init scripts, `cron`, etc.  (It's harmless to run if the server is already up.)
* `bin/link_off foreground` will start LinkOff in the current session, not in the background.  This isn't very useful if you're running it from the command line, but it's great for tools like `runit`.
